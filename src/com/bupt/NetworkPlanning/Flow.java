...
public void setOutputPort(int outputPort) {
  ...
  public String toXmlString( String srcMac,String dstMac ){
	  StringBuilder sb = new StringBuilder();
	  sb.append("<?xml version="+"\""+"1.0"+"\""+" encoding="+"\""+"UTF-8"+"\""+ " standalone="+"\""+"no"+"\""+"?>");
	  sb.append("<flow xmlns="+"\""+"urn:opendaylight:flow:inventory"+"\""+">"+"<id>");
	  sb.append(this.getFlowId());
	  sb.append("</id><table_id>0</table_id><flow-name>Foo</flow-name><match><ethernet-match><ethernet-source><address>");
	  sb.append(srcMac);
	  sb.append("</address></ethernet-source> <ethernet-destination> <address>");
	  sb.append(dstMac);
	  sb.append("</address></ethernet-destination> "
	  		+ "</ethernet-match></match> <priority>60</priority> <instructions>     "
	  		+ " <instruction><order>0</order><apply-actions><action> "
	  		+ "<order>0</order><output-action><output-node-connector>");	
       sb.append(this.getOutputPort()) ;    
       sb.append("</output-node-connector><max-length>65535</max-length>  "
